...
const App = () => {
  ...
  const fetchProducts = async () => {
    if (!apiUrl) {
      console.error("API URL tanımlanmamış. Lütfen REACT_APP_API_URL ortam değişkenini kontrol edin.");
      setLoading(false);
      return;
    }
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Content style={{ padding: '24px' }}>
        <div style={{ background: '#fff', padding: 24, borderRadius: 8 }}>
          ...
          {!apiUrl && <div style={{color: 'red', marginBottom: '20px', fontWeight: 'bold'}}>HATA: API Adresi (REACT_APP_API_URL) yapılandırılmamış.</div>}
